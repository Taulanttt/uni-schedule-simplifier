...
/* special value Radix is happy with */
const ALL = "__all__";

/* labels */
const LABEL_ALL_YEAR   = "Të gjithë vitet ak.";
const LABEL_ALL_AFATI  = "Të gjithë afatet";
const LABEL_ALL_STUDY  = "Të gjitha vitet";
...
const FilterPanelExams:React.FC<Props> = ({ filters, setFilters, compact=false }) => {
  ...
  /* map UI value → state value ("" means “no filter”) */
  const handle = (key:keyof FilterOptionsexam)=>(v:string)=>{
    setFilters(f=>({...f, [key]: v===ALL? "" : v}));
  };

  /* map state value → value prop the <Select> expects */
  const mapVal = (v:string)=> v === "" ? ALL : v;
  ...
  if (compact){
    return (
      <div className="flex items-center gap-2">
        <Field
          value={mapVal(filters.academicYear)}
          onChange={handle("academicYear")}
          placeholder="Viti ak."
          width="w-[140px]"
        >
          <SelectItem value={ALL}>{LABEL_ALL_YEAR}</SelectItem>
          {years.map(y=> <SelectItem key={y.id} value={y.name}>{y.name}</SelectItem>)}
        </Field>

        <Field
          value={mapVal(filters.afati)}
          onChange={handle("afati")}
          placeholder="Afati"
          width="w-[120px]"
        >
          <SelectItem value={ALL}>{LABEL_ALL_AFATI}</SelectItem>
          {afati.map(a=> <SelectItem key={a.id} value={a.name}>{a.name}</SelectItem>)}
        </Field>

        <Field
          value={mapVal(filters.yearOfStudy)}
          onChange={handle("yearOfStudy")}
          placeholder="Viti"
          width="w-[110px]"
        >
          <SelectItem value={ALL}>{LABEL_ALL_STUDY}</SelectItem>
